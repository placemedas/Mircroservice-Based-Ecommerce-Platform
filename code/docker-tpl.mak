...
deploy: product db logger
	...
	$(DK) run -t --publish 30002:30002 --detach --name cart $(CREG)/$(REGID)/cart:e3 | tee cart.svc.log
	$(DK) run -t --publish 30000:30000 --detach --name customer $(CREG)/$(REGID)/customer:e3 | tee customer.svc.log
	$(DK) run -t \
...
$(LOG_DIR)/cart.repo.log: cart/appd.py cart/Dockerfile
	...
	$(DK) build -t $(CREG)/$(REGID)/cart:e3 cart | tee $(LOG_DIR)/cart.img.log
	$(DK) push $(CREG)/$(REGID)/cart:e3 | tee $(LOG_DIR)/cart.repo.log

$(LOG_DIR)/customer.repo.log: customer/appd.py customer/Dockerfile
	cp customer/appd.py customer/app.py
	$(DK) build -t $(CREG)/$(REGID)/customer:e3 customer | tee $(LOG_DIR)/customer.img.log
	$(DK) push $(CREG)/$(REGID)/customer:e3 | tee $(LOG_DIR)/customer.repo.log
